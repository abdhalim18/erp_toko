...
-- Create kategori table (no dependencies)
...
-- Create pemasok table (no dependencies)
...
-- Create produk table (depends on kategori and pemasok)
CREATE TABLE `produk` (
  `id` VARCHAR(36) NOT NULL,
  ...
  KEY `produk_id_kategori_fkey` (`id_kategori`),
  KEY `produk_id_pemasok_fkey` (`id_pemasok`),
...
-- Create penjualan table (no foreign key dependencies)
...
-- Create detail_penjualan table (depends on penjualan and produk)
